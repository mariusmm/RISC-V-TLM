...
namespace riscv_tlm {
    ...
    void BusCtrl::b_transport(tlm::tlm_generic_payload &trans,
        ...
        (*decodeSocket)->b_transport(trans, delay);
    ...
    unsigned int BusCtrl::encode(const sc_dt::uint64 st_address, const sc_dt::uint64 end_address) {
        ...
        if (it == peripherals_map.end()) {
            ...
            std::cout << "New peripheral: Address 0x" << std::hex << to_insert.s << ", 0x" << to_insert.e << " to port #" << std::dec << peripherals_count << std::endl;
    ...
    unsigned int BusCtrl::register_peripheral(sc_dt::uint64 start, sc_dt::uint64 end) {
